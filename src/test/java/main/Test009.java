// Тест №009
// Проверка функционала 'Оценка заказа' и перенаправления в Google Play Market

package main;

import org.openqa.selenium.By;

public class Test009 extends Main {

    public static final String NAME = "Тест №009 - Проверка функционала 'Оценка заказа' и перенаправления в " +
            "Google Play Market";

    public static void test009(String deviceName) {
        ...
        // Проставляем заказу оценку 5 звезд
        selectRating("5");

        // Жмем кнопку "ОТПРАВИТЬ", чтобы отправить оценку
        clickSendRatingButton();

        // Проверяем наличие алерта "Если вам нравится приложение, оцените его!"
        assertRatingAlert();

        // Жмем кнопку "ОЦЕНИТЬ" на алерте
        clickEstimateButton();

        // Убеждаемся, что мы перешли в Google Play Market на страницу приложения
        assertGooglePlayMarket();

        // Переключаемся обратно на наше приложение
        // driver.launchApp();
    }
}
